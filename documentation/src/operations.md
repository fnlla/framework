...
**-** Run the scheduler (`php bin/fnlla schedule:run`) via cron/worker.
...
**-** `Fnlla_WARM_KERNEL=1` for long-running servers
...
`Fnlla\\Contracts\Log\ErrorReporterInterface` in the container. The core
...
**-** Run migrations after deploy: `php bin/fnlla migrate`.
**-** Warm provider cache: `php bin/fnlla-discover` (if needed).
...
**-** Run them via path override (`php bin/fnlla migrate --path=database/migrations-baseline`) or `MIGRATIONS_PATH`.
**-** Generate all project-specific migrations with `php bin/fnlla make:migration <name>` in your main migrations path.
...
**CACHES**
**-** Provider discovery cache: `bootstrap/cache/providers.php` (generated by `bin/fnlla-discover`).
...
Generate routes cache via your CLI or `Fnlla\\Http\RouteCacheCompiler`.
...
Generate it with your CLI (for app package: `php cli/fnlla.php routes:cache`) or `Fnlla\\Http\RouteCacheCompiler`.
...
**-** For long-running servers, set `Fnlla_WARM_KERNEL=1`.
**-** Protect docs with `Fnlla_DOCS_PUBLIC=0` and `Fnlla_DOCS_ACCESS_TOKEN` (or disable `Fnlla_DOCS_ENABLED=0`).
...
**-** If Admin is enabled, set `Fnlla_ADMIN_LOGIN_EMAIL` and `Fnlla_ADMIN_LOGIN_PASSWORD_HASH`, and keep `Fnlla_ADMIN_LOGIN_REQUIRED=1`.

**ENV KEY MIGRATION (LEGACY -> CURRENT)**
**-** `APP_WARM_KERNEL` -> `Fnlla_WARM_KERNEL`
**-** `DOCS_ENABLED` -> `Fnlla_DOCS_ENABLED`
**-** `DOCS_PUBLIC` -> `Fnlla_DOCS_PUBLIC`
**-** `DOCS_ACCESS_TOKEN` -> `Fnlla_DOCS_ACCESS_TOKEN`
**-** `ADMIN_LOGIN_REQUIRED` -> `Fnlla_ADMIN_LOGIN_REQUIRED`
**-** `ADMIN_LOGIN_EMAIL` -> `Fnlla_ADMIN_LOGIN_EMAIL`
**-** `ADMIN_LOGIN_PASSWORD_HASH` -> `Fnlla_ADMIN_LOGIN_PASSWORD_HASH`
...
```bash
php bin/fnlla docs:generate --publish --app=.
```
Publish to a custom target:
```bash
php bin/fnlla docs:generate --publish --publish-target=resources/docs
```
...
This is the default workflow for working on fnlla (finella).
...
**SECURITY NOTES**
**-** Disable docs entirely in production: `Fnlla_DOCS_ENABLED=0`.
...
**-** Ensure providers are auto-discovered via `extra.fnlla.providers`.
...
```json
{
  ...
  "repositories": [
    {"type": "vcs", "url": "git@github.com:techayo/fnlla-ai.git"}
  ],
  "require-all": true
}
```
...
**Deprecated:** `Fnlla\\Support\CacheStoreInterface`
**Replacement:** `Fnlla\\Contracts\Cache\CacheStoreInterface`

**WHAT CHANGED**
Cache store contracts moved into the `Fnlla\\Contracts\Cache` namespace to
...
**Deprecated:** `Fnlla\\Support\ArrayCacheStore`
**Replacement:** `Fnlla\\Cache\ArrayStore`
...
**EXAMPLE**
```php
use Fnlla\\Cache\ArrayStore;
```

**MIGRATION DEP-2026-03**

**Deprecated:** `Fnlla\\Support\FileCacheStore`
**Replacement:** `Fnlla\\Cache\FileStore`
...
**EXAMPLE**
```php
use Fnlla\\Cache\FileStore;
```

**MIGRATION DEP-2026-04**

**Deprecated:** `Fnlla\\Support\Queue`
**Replacement:** `Fnlla\\Queue\SyncQueue`
...
**MIGRATION STEPS**
**-** Update class references to `Fnlla\\Queue\SyncQueue`.
