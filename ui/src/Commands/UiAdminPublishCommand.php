...
final class UiAdminPublishCommand implements CommandInterface
{
    public function getName(): string
    {
        return 'ui:admin:publish';
    }

    public function getDescription(): string
    {
        return 'Publish Finella UI admin stubs into the app.';
    }
    ...
    public function run(array $args, array $options, ConsoleIO $io, string $root): int
    {
        ...
        $source = dirname(__DIR__, 2) . '/admin-stubs';
        if (!is_dir($source)) {
            $io->error('UI admin stubs not found: ' . $source);
            return 1;
        }
